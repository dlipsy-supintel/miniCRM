...
export async function GET() {
  ...
  try {
    const state = signState({ org_id: profile.org_id, user_id: user.id, provider: 'calendly' })
    return NextResponse.redirect(await getCalendlyOAuthUrl(state, profile.org_id))
  } catch (err) {
